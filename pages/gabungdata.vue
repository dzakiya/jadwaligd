...
<template>
    <div class="d-flex flex-no-wrap justify-space-between">
      <v-row align="center" justify="center" dense>
        <div v-for="(ksm, i) in jmlKsm.data" :key="i">
        <div v-for="(item, id) in jadwal.data" :key="item.id">
        <div v-if="ksm.Nama_ksm === item.Ksm.Nama_ksm">
  ...
  <br><h1>Gabung Data belum hapus duplikasi</h1>
  <pre>{{ gabungJadwal.data.length }}</pre>
  <br>
  <h1>Combined Data Hapus Duplikasi </h1>
  ...
  //const jadwal = await useFetch(`https://satu.dev.rssa.id/items/data_jadwal_jaga_dokter?fields=id, Nama_petugas,Ksm.Nama_ksm,Level.Nama_level_igd,Jaga_awal,Jaga_akhir&filter[Level][_neq]=3`);
  const jmlKsm = await $fetch(`https://satu.dev.rssa.id/items/daftar_ksm`);
  
  //COBA PAKAI CARA INI 
  const { data : jadwal1 } = useFetch(`https://satu.dev.rssa.id/items/data_jadwal_jaga_dokter?fields=id,Nama_petugas,Ksm.Nama_ksm,Level.Nama_level_igd,Jaga_awal,Jaga_akhir&filter[Jaga_awal][_gte]=$NOW&filter[Jaga_akhir][_gte]=$NOW
  `);
  
  const { data : jadwal2 }  = useFetch(`https://satu.dev.rssa.id/items/data_jadwal_jaga_dokter?fields=id, Nama_petugas,Ksm.Nama_ksm,Level.Nama_level_igd,Jaga_awal,Jaga_akhir&filter[Jaga_awal][_lte]=$NOW&filter[Jaga_akhir][_gte]=$NOW
  `);
  
  const gabungJadwal = [].concat(jadwal1, jadwal2); // belum menghapus duplikasi
  
  const combinedData = [].concat(jadwal1, jadwal2).reduce((uniqueItems, item) => { //hapus duplikasi
