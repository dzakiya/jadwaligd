...
const { data: dataJadwal } = await useFetch(
  `https://satu.dev.rssa.id/items/data_jadwal_jaga_dokter?fields=id,Nama_petugas,Ksm.Nama_ksm,Level.Nama_level_igd,Jaga_awal,Jaga_akhir&filter[day(Jaga_awal)][_eq]=${tgl}&filter[month(Jaga_awal)][_eq]=${bln}&filter[year(Jaga_awal)][_eq]=${thn}`
);
...
const jadwal = await $fetch(`https://satu.dev.rssa.id/items/data_jadwal_jaga_dokter?fields=Nama_petugas,Ksm.Nama_ksm,Level.Nama_level_igd,Jaga_awal,Jaga_akhir&filter[Jaga_awal][_lte]=$NOW&sort[]=-Jaga_awal`)
