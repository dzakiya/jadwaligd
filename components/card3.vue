<template>

  <div class="d-flex flex-no-wrap justify-space-between">
    <v-row align="center" justify="center" dense>
      <p v-for="(ksm, i) in jmlKsm.data" :key="i">
      <p v-for="(item, id) in jadwal.data" :key="item.id">
      <p v-if="ksm.Nama_ksm === item.Ksm.Nama_ksm">
        <p v-if="item.Jaga_awal <=  formattedTime  && item.Jaga_akhir >=  formattedTime">
         <!--FORMAT TANGGAL-->
        <v-col cols="12" md="3">
          <v-card variant="tonal" width="300" color="surface-variant" :title="ksm.Nama_ksm"
            append-avatar="data:image/jpeg;base64,">
            <v-list>
              <v-list-item :key="item.id" :subtitle="item.Nama_petugas" :title="item.Level.Nama_level_igd">
                <template v-slot:prepend>
                  <v-avatar color="blue">
                    <v-icon color="white" icon="mdi-hospital-box"></v-icon>
                  </v-avatar>
                </template>
              </v-list-item>

              <v-list-item :key="item.id" :subtitle="item.Jaga_awal">
              </v-list-item>
              <v-list-item :key="item.id" :subtitle="item.Jaga_akhir">
              </v-list-item>

            </v-list>
          </v-card>
        </v-col>
      </p> 
    
      </p>
      </p>
      </p>

    </v-row>
  </div>

 {{ formattedTime }}
</template>

<script setup>
import { useNow, useDateFormat } from "@vueuse/core";

const variants = ["elevated", "flat", "tonal", "outlined"];
const slides = ["First", "Second", "Third", "Fourth", "Fifth"];

//untuk filter tgl
const now = useDateFormat(useNow(), "dddd, DD MM YYYY HH:mm:ss", {
  locales: "id-ID",
});

//untuk pengecekan date time
const realTime = new Date();
const formattedTime = `${realTime.getFullYear()}-${pad(realTime.getMonth() + 1)}-${pad(realTime.getDate())}T${pad(realTime.getHours())}:${pad(realTime.getMinutes())}:${pad(realTime.getSeconds())}`;

function pad(number) {
  return number < 10 ? `0${number}` : number.toString();
}

const tgl = ref(0);
const bln = ref(0);
const thn = ref(0);

const tanggal = useDateFormat(useNow(), "DD", { locales: "id-ID" });
const bulan = useDateFormat(useNow(), "MM", { locales: "id-ID" });
const tahun = useDateFormat(useNow(), "YYYY", { locales: "id-ID" });

tgl.value = tanggal.value;
bln.value = bulan.value;
thn.value = tahun.value;

//const jadwal = await $fetch(`https://satu.dev.rssa.id/items/data_jadwal_jaga_dokter?fields=id,Nama_petugas,Ksm.Nama_ksm,Level.Nama_level_igd,Jaga_awal,Jaga_akhir&filter[day(Jaga_awal)][_eq]=15&filter[month(Jaga_awal)][_eq]=05&filter[year(Jaga_awal)][_eq]=2024`);

const jadwal = await $fetch(`https://satu.dev.rssa.id/items/data_jadwal_jaga_dokter?filter[Jaga_awal][_between]=['2024-05-16','2024-05-18']&fields=id, Nama_petugas,Ksm.Nama_ksm,Level.Nama_level_igd,Jaga_awal,Jaga_akhir`)

const jmlKsm = await $fetch(`https://satu.dev.rssa.id/items/daftar_ksm`);

//get data lte gte
const { data: jadwal1 } = await useFetch(
  `https://satu.dev.rssa.id/items/data_jadwal_jaga_dokter?fields=id,Nama_petugas,Ksm.Nama_ksm,Level.Nama_level_igd,Jaga_awal,Jaga_akhir&filter[Jaga_awal][_lte]=$NOW&filter[Jaga_akhir][_gte]=$NOW
`
);

//get data gte gte
const { data: jadwal2 } = await useFetch(
  `https://satu.dev.rssa.id/items/data_jadwal_jaga_dokter?fields=id,Nama_petugas,Ksm.Nama_ksm,Level.Nama_level_igd,Jaga_awal,Jaga_akhir&filter[Jaga_awal][_gte]=$NOW&filter[Jaga_akhir][_gte]=$NOW
`
);

// const dataJadwal1 = ref([]);
// const dataJadwal2 = ref([]);
// const dataJadwal = ref([]);

// // dataJadwal1.value = jadwal1.data;
// // dataJadwal2.value = jadwal2.data;

// const data1 = jadwal1.data;
// const data2 = jadwal2.data;

// //gabungkan array dataJadwal 
// dataJadwal.value = [].concat(jadwal1, jadwal2);

</script>
